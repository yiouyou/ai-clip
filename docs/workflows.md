...
`review` 导出的 `script.md` 按镜头分 block：所有体裁可编辑口播；slideshow 额外使用
`Caption: ...` 行编辑或清空画面标题；remix 可修改 `[start-end]` 秒数或删除整个 block。
新生成的 storyboard 会保存 `target_duration_sec`。`review --apply` 若发现 remix 编辑后的
时间戳总长超过目标，会返回 exit code 2 并保持原 `storyboard.json` 不变；没有该字段的旧
storyboard 按兼容模式应用，不额外限制。
...
ai-clip remix "<url>" -p demo --theme "..." --duration 60 --research --research-searches 1
...
`remix --duration` 作为硬上限参与两次收敛：storyboard 对 LLM 选出的 source spans 按比例
缩短，保留所有选题位置而不是从尾部截断；assemble 将超出对应 span 的 TTS 通过 FFmpeg
`atempo` 链压回镜头时长。提示词同时限制逐镜头口播字数，正常新稿应优先依靠短文案，
tempo 处理只是确定性兜底。talking-head、slideshow、montage 不受此规则影响。
...
- ComfyUI 生成会在图片旁写 `.job.json`；中断后按原 `prompt_id` 恢复，图片已落盘但
  manifest 尚未写入时也会自动补齐。
